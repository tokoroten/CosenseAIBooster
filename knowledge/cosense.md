...
## 現在の問題点
- バックエンドでの設定変更（特にプロンプトの追加・更新・削除）がフロントエンドストアに自動的に反映されない
- フロントエンドは起動時に一度だけ設定をロードするため、設定変更後にページをリロードするか拡張機能を再起動しないと新しい設定が反映されない

## 解決策
- Chrome Storageの変更イベント（`storage.onChanged`）をリッスンして、変更があった場合にフロントエンドストアを自動的に更新する
- または、メッセージングシステムを導入し、バックエンドで設定が変更された際にすべてのフロントエンドに通知を送信する
