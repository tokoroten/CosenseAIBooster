...
- 設定画面の見直し
  -　ダイアログの横幅が狭いので、横幅を広げる　→　対応済み
  - 同じ内容がヘッダに多重に表示されているので、ヘッダを削除する　→　対応済み
  - APIキーを検証するボタンを置く　→　対応済み
...
## 対応中
- [ ] トップレベルに多くのファイルが転がりすぎているので、コード関連のファイルはsrcフォルダを掘って、その下に移動させ、またビルドスクリプトもそれに合わせて修正しなさい。
- [x] WXTの作法に従って、過去に作成されたコードを整理していって（src配下は整理済み、トップレベル整理は未完了）
