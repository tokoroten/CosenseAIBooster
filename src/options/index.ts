...
function updatePromptList(prompts) {
  ...
  if (prompts.length === 0) {
    promptList.innerHTML = '<div class="p-4 text-center text-gray-500">プロンプトがありません</div>';
    return;
  }
  
  prompts.forEach((prompt, index) => {
    const promptItem = document.createElement('div');
    promptItem.className = 'flex items-center p-3 border-b border-gray-200 hover:bg-gray-50 cursor-pointer';
